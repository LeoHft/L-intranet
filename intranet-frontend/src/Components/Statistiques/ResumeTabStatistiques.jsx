        ...
        <div className="card bg-base-100 shadow-xl">
          <div className="card-body">
            <div className="card-title justify-between mb-6">
              <h3 className="text-2xl">Tableau des statistiques</h3>
            </div>
              ...
              <table className="table table-zebra">
                <thead>
                  <tr>
                    ...
                    <th>Utilisateur</th>
                    <th>Date</th>
                    <th>Accès internes</th>
                    <th>Accès externes</th>
                    <th>Total</th>
                  </tr>
                </thead>
                <tbody>
                  {statisticsData.map((stat, index) => (
                    <tr key={index} className="hover">
                      ...
                      <td>{stat.user_name || `Utilisateur ${stat.user_id}`}</td>
                      <td>
                        {new Date(stat.click_date).toLocaleDateString("fr-FR")}
                      </td>
                      <td>{stat.internal_url_click}</td>
                      <td>{stat.external_url_click}</td>
                      <td className="font-medium">
                        {parseInt(stat.internal_url_click) +
                          parseInt(stat.external_url_click)}
                      </td>
                    </tr>
                  ))}
                </tbody>
              ...
              <div className="stats stats-vertical lg:stats-horizontal shadow mt-6">
                <div className="stat">
                  ...
                  <div className="stat-value text-primary">
                    {statisticsData.reduce(
                      (sum, stat) => sum + parseInt(stat.internal_url_click),
                      0
                    )}
                  </div>
                </div>
                <div className="stat">
                  ...
                  <div className="stat-value text-secondary">
                    {statisticsData.reduce(
                      (sum, stat) => sum + parseInt(stat.external_url_click),
                      0
                    )}
                  </div>
                </div>
