...
const generateColors = (count) => {
    ...
    for (let i = 0; i < count; i++) {
        if (i < baseColors.length) {
            ...
            const fill = `hsl(${finalHue}, ${Math.max(50, saturation - 10)}%, ${lightness}%)`;
...
export default function ByServicesStackAreaStatistiques({ statisticsData }) {
    ...
    const colors = useMemo(() => generateColors(serviceNames.length), [serviceNames.length]);
    ...
    const displayedServices = showAll ? serviceNames : serviceNames.filter(service => visibleServices.has(service));
