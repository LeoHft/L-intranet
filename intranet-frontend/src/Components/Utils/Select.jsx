...
export default function CustomSelect({ options, selectedOption, setSelectedOption, name, placeholder, required = false }) {

    return (
        <div className="form-control">
            <label className="label-text font-medium">{name}{required && <span className="text-error">*</span>}</label>
                <Select
                    options={options}
                ...
                {required && (!selectedOption || selectedOption.length === 0) && (
                    <p className="text-sm text-error">Au moins une option doit être sélectionnée</p>
                )}
        </div>
    );
}
